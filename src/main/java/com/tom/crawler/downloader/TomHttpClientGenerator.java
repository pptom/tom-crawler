...
public class TomHttpClientGenerator {
    ...
    private SSLConnectionSocketFactory buildSSLConnectionSocketFactory() {
        ...
        try {
            ...
            logger.warn("get sslContext instance error! {}", e.getMessage());
        }
        try {
            sslContext.init(null, xtmArray, new SecureRandom());
        } catch (KeyManagementException e) {
            logger.warn("init sslcontext error! {}", e.getMessage());
        }
        String[] supportedProtocols = new String[]{"TLSv1", "TLSv1.1", "TLSv1.2"};
        SSLConnectionSocketFactory sslsf = new SSLConnectionSocketFactory(
                sslContext, supportedProtocols, null,
                //客户端验证服务器身份的策略
    ...
    public TomHttpClientGenerator setPoolSize(int poolSize) {
